...
A1,p1 = largest_component(A[index_id,index_id]);
...
function FindCommunity(Sig::AbstractArray{Float64,2},R0::Array{Float64,2})
...
node_names = unique(w_adjacency_list[:,1]);
...
index = RandomGenerateVec(p,n_clu); #index = rand(1:N,n_clu);
...
#community = Array{Int64,1}(N);
community = FindCommunity(Sig,R0);# Gives as many communities as nodes

# Then, we update the centroids
R = zeros(Float64,dim,n_clu);
R,community = UpdateCentroids(Sig,R0,community);
...
n_updates::Int64 = 0;
community  = Reorder(vec(community));
n_c = length(unique(community));
...
for it =1:it_max   
R1,community1 = UpdateCentroids(Sig,R,community);
community1  = Reorder(vec(community1));
...
function SphereEmbedCluster(A::SparseMatrixCSC{Int64,Int64},n_it_PPM::Int64,t::Float64, n_clusters::Int64,n_it_Kmeans::Int64,n_updates::Int64,shape::String,r0::Int64)
...
#H0 = Array{Float64,2}(N,r0);
H0 = A[:,index]-(1/s)*d*d[index]';
H0 = ProjectSphere(H0);

print("\n")
print(" ------- Acc Projected Power Iteration -------\n")
fast::Int64 = 1;
H0 = @time AccProjPowMethodPreProcessed(A,H0,n_it_PPM,t,fast);
U, S, ~ = svd(H0);
print(" -------------- Clustering ------- \n")
dim::Int64 = length(S);
print("dimension of embedding used for clustering: ")
print(dim)
print("\n")
...
print("\n")
print(" -------------------------------------------- \n")
print("The squared singular values divided by N : \n")
print((S.^2)/N)
print("\n")
...
function UpdateCentroids(S::AbstractArray{Float64,2},R0::Array{Float64,2},community::Array{Int64,1})
...
function Weighted_Adjacency_List(w_edge_list::Array{Int64,2})
    ...
    for l =1:n_connections
        i = w_edge_list[l,1];index_i = find(node_names.==i)[1];
        w = w_edge_list[l,3];
